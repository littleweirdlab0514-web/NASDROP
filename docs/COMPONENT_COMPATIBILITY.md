...
| --- | --- | --- |
| Synology package | 0.9.26-3 security test build | Canonical server, DSM-authenticated launcher handoff with ordinary NASDrop login fallback for configured accounts, provider security, API, and web UI |
| Docker image | 0.9.26-3 pending | Source version synchronized; build and smoke-test only after the Synology security flow passes |
