...
- Develop and verify shared server/provider changes in the private Docker test deployment first, coordinating Chrome-owned changes with its dedicated task. After regressions and the real Docker provider flow pass, build the Synology SPK from the same canonical source and verify DSM-specific behavior. Never maintain a Docker-only provider fork.
...
- The user changed the workflow on 2026-09-26: Docker is the first implementation and real-test environment, not a downstream port. Coordinate the exact shared changes, API capabilities, version, tests, and migration notes with the Docker maintainer. Preserve existing accounts, jobs, and storage during test updates.
- The required sequence is: shared implementation and automated tests -> private Docker test build/deployment and real-flow verification -> versioned SPK build from the same source -> user installs and verifies DSM packaging, permissions, launcher, and lifecycle -> release gates and promotion. Chrome-owned work remains routed through `NASDROP_크롬확장`. Docker success does not prove DSM-specific compatibility; do not install the SPK without authorization.
...
- Mandatory user-facing invariant (reaffirmed 2026-09-26): on servers advertising `job_safe_delete`, users must be able to delete an active job with the Delete button alone, without manually pausing first. The UI must enable deletion for eligible active selections and send `stop_active: true`; the server safely stops the worker, confirms ownership has ended, then automatically removes the selected record, secrets, and temporary workspace. Show pending shutdown honestly and prevent duplicate actions. Retain published files. Require UI/API regression coverage and Docker-first real-flow verification so a server-only implementation cannot silently regress to pause-before-delete behavior. Apply the same capability contract to Android and Chrome job-management clients where supported.
